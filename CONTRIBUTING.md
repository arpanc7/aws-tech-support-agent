...
4. Preserve bounded grounding: unsupported questions must abstain, every displayed claim must cite active stored evidence, and citations/URLs must be built by the server. Multi-service retrieval must remain balanced, and bare-comparison claims must cite every named service. Keep the single optional search round and do not add autonomous tools or unbounded loops.
