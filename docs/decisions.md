...
Status: implemented baseline with release gates pending · Design baseline: 0.4 · Updated: 2026-09-02
...
| ADR-009 (updated 2026-09-02) | Bounded grounded synthesis | The user approved a simpler Agentic RAG flow: one research decision, at most one additional local search round with up to three queries, one cited answer draft, and one grounding review. Java validates all model-selected aliases and builds citations. Multi-service questions retrieve each named service independently and interleave their rankings. Bare-comparison claims must cite every named service. This improves usefulness but retains residual same-model hallucination risk, so human evaluation remains a release gate. |
