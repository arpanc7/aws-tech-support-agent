...
| A-11a Named service scope | B-04 | With an empty filter, “What is EC2?” retrieves only EC2 evidence. “Compare EC2 with Lambda” retrieves both service rankings independently, interleaves them, and answers from both local overviews. A question with no named service remains globally scoped. |
| A-11b Comparison relevance | B-22 | A bare EC2/Lambda comparison retains claims only when their citations cover both services. One-sided pricing, quota, or product-variant tangents are removed; zero retained comparisons abstain. |
