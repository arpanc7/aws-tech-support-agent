...
**Historical trace:** captured on 2026-08-31 before the LangChain4j integration. Numbers and local artifact links below describe that recorded run, not a new post-migration measurement. The same two-stage policy now uses EvidencePromptChain/PromptStage with packaged templates and a guarded ChatModel bridge; see [current low-level design](low-level-design.md) and [verification status](implementation-status.md). Prompt content was preserved, and answer-profile identity was updated. A post-migration Lambda trace confirmed identical embedding and both generation request JSON payloads; its separate artifacts are under `.cache/langchain4j/trace` with comparison results in `.cache/langchain4j/wire-comparison.json`.
